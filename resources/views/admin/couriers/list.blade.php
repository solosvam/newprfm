@php
    $html_tag_data = [];
    $title = 'Kuryerlər';
    $breadcrumbs = ["/"=>"ParfumShop", ""=>"Kuryerlər"]
@endphp
                        ...
                        <div class="card-body">
                            ...
                            <table class="data-table data-table-pagination data-table-standard responsive nowrap hover" id="datatableHover" data-order='[[ 0, "desc" ]]'>
                                ...
                                <tr>
                                    ...
                                    <th class="text-muted text-small text-uppercase">SOYAD</th>
                                    <th class="text-muted text-small text-uppercase">EMAIL</th>
                                    <th class="text-muted text-small text-uppercase">MOBİL</th>
                                ...
                                @foreach($couriers as $key => $courier)
                                    <tr>
                                        <td>{{$key+1}}</td>
                                        <td>{{$courier->name}}</td>
                                        <td class="text-alternate">{{$courier->surname}}</td>
                                        <td class="text-alternate">{{$courier->email}}</td>
                                        <td class="text-alternate">{{$courier->mobile}}</td>
                                        <td class="text-alternate">{{$courier->active ? 'Aktiv' : 'Deaktiv' }}</td>
                                        <td class="text-alternate">
                                            <a href="{{route('courier.edit',$courier->id)}}" class="btn btn-primary btn-sm">Edit</a>
                                        </td>
                                    </tr>
                                @endforeach
                                </tbody>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">Yeni əməkdaş</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <form method="POST" action="{{route('courier.add')}}">
                            @csrf
                            <label>Ad</label>
                            ...
                            <label>Mobil</label>
                            <input type="text" name="mobile" class="form-control" placeholder="Mobil nömrə" value="{{old('mobile')}}" required>
                            <label>Parol</label>
