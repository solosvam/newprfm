...
<main>
 <div class="container">
  <div class="cabinet">
   ...
   @if($orders->isEmpty())
    <div class="cabinet-content-empty"><h2>{{ __('orders_history') }}</h2><p>{{ __('profile_you_have_no_orders_yet') }}</p></div>
   @else
    <div class="order-history">
     @foreach($orders as $order)
      <div class="order-card">
       <div class="order-card__head">
        <div><span class="order-card__label">{{ __('orders_order_date') }}</span><span class="order-card__value">{{ $order->created_at->format('d.m.Y') }}</span></div>
        <div><span class="order-card__label">{{ __('orders_order_summary') }}</span><span class="order-card__value">{{ $order->items->sum('quantity') }} {{ __('orders_products') }}</span></div>
        <div><span class="order-card__label">{{ __('orders_order_no') }}</span><span class="order-card__value">{{ $order->order_no }}</span></div>
        <div><span class="order-card__label">{{ __('orders_total') }}</span><span class="order-card__total">{{ number_format($order->total,2) }} ₼</span></div>
       ...
       <div class="order-card__body">
        ...
        <div class="order-card__products">
         ...
         @endforeach
         @if($order->items->count()>4)<span class="order-card__more">+{{ $order->items->count()-4 }} {{ __('orders_products') }}</span>@endif
        </div>
       </div>
      </div>
     @endforeach
