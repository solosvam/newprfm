...
<div class="bonus-history">
 <div class="bonus-head"><h2>{{ __('bonus_bonus_history') }}</h2><div class="bonus-balance">{{ __('bonus_bonus_balance') }} <strong>{{ number_format(auth()->user()->bonus_balance ?? 0,2) }} ₼</strong></div></div>
 @if($transactions->isEmpty())<p>{{ __('bonus_no_bonus_transactions_yet') }}</p>
 @else
 <table class="bonus-table">
  <thead><tr><th>{{ __('orders_order_no') }}</th><th>{{ __('bonus_date') }}</th><th>{{ __('bonus_transaction') }}</th><th>{{ __('bonus_bonus_amount') }}</th></tr></thead>
  <tbody>
  @foreach($transactions as $transaction)
   <tr>
    ...
    <td>{{ $transaction->type === 'earn' ? __('orders_bonus_earned') : ($transaction->type === 'spend' ? __('bonus_bonus_spent') : ($transaction->note ?: __('bonus_bonus_transaction'))) }}</td>
