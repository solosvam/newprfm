<div class="cabinet__aside">
    <div class="cabinet__aside__top"><ul>
        <li><a href="{{ url()->previous() }}"><img src="{{ asset('frontend/images/arrow-left.svg') }}" alt=""><span>{{ __('cart_go_back') }}</span></a></li>
        <li><ul><li><a href="{{ route('home') }}"><img src="{{ asset('frontend/images/home.svg') }}" alt=""></a></li><li><img src="{{ asset('frontend/images/arrow-right.svg') }}" alt=""></li><li><a href="{{ route('profile') }}">{{ __('auth_my_account') }}</a></li>@isset($pageTitle)<li><img src="{{ asset('frontend/images/arrow-right.svg') }}" alt=""></li><li><span>{{ $pageTitle }}</span></li>@endisset</ul></li>
    </ul></div>
    <div class="cabinet__aside__nav">
        <h1>{{ __('navigation_hello_name', ['name' => trim(auth()->user()->name.' '.auth()->user()->surname)]) }}</h1>
        <ul>
            <li><a href="{{ route('profile') }}" class="{{ request()->routeIs('profile') ? 'cabinet-active' : '' }}">{{ __('catalog_account_information') }}</a></li>
        ...
        <form method="POST" action="{{ route('front.logout') }}" class="cabinet-logout-form">@csrf
            <button type="submit" class="cabinet-logout"><img src="{{ asset('frontend/images/signin.svg') }}" alt=""><span>{{ __('catalog_sign_out') }}</span></button>
        </form>
    </div>
</div>