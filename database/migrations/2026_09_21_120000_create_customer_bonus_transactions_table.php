...
return new class extends Migration {
 public function up(): void {
  ...
  Schema::create('customer_bonus_transactions',function(Blueprint $t){$t->id();$t->unsignedInteger('customer_id');$t->unsignedBigInteger('order_id')->nullable();$t->enum('type',['earn','spend','register','adjustment']);$t->decimal('amount',12,2);$t->string('note')->nullable();$t->unsignedBigInteger('created_by')->nullable();$t->timestamps();$t->foreign('customer_id')->references('id')->on('customers')->cascadeOnDelete();$t->foreign('order_id')->references('id')->on('orders')->nullOnDelete();});
 }
