...
class CheckoutController extends Controller {
 ...
 public function store(Request $request){
  ...
  return DB::transaction(function()use($data,$customer){
   ...
   $variants=ProductVariant::with(['product.brand','size'])->whereIn('id',$cart->keys())->where('active',1)->get();
   abort_if($variants->count()!==$cart->count(),422,'Səbətdə mövcud olmayan məhsul var.');
   $subtotal=0;$items=[];
   foreach($variants as $v){$qty=(int)$cart[$v->id]['quantity'];$line=round((float)$v->price*$qty,2);$subtotal+=$line;$items[]=['product_id'=>$v->product_id,'product_variant_id'=>$v->id,'product_name'=>$v->product->name,'brand_name'=>$v->product->brand?->name,'size_name'=>$v->size?->name_az,'unit_price'=>$v->price,'quantity'=>$qty,'total'=>$line];}
   $order=Order::create(['order_no'=>'PS'.now()->format('ymd').str_pad((string)((Order::max('id')??0)+1),6,'0',STR_PAD_LEFT),'customer_id'=>$customer->id,'customer_address_id'=>$address->id,'payment_method_id'=>$data['payment_method_id'],'source'=>'website','order_status_id'=>$initialStatus->id,'status'=>$initialStatus->code,'customer_name'=>$customer->name,'customer_surname'=>$customer->surname,'customer_mobile'=>$customer->mobile,'address_title'=>$address->title,'city'=>$address->city,'district'=>$address->district,'address'=>$address->address,'building'=>$address->building,'entrance'=>$address->entrance,'floor'=>$address->floor,'apartment'=>$address->apartment,'address_note'=>$address->note,'gift_wrap'=>(bool)($data['gift_wrap']??false),'customer_note'=>$data['customer_note']??null,'subtotal'=>$subtotal,'discount'=>0,'total'=>$subtotal]);
   $order->items()->createMany($items);DB::table('order_status_logs')->insert(['order_id'=>$order->id,'status'=>$initialStatus->code,'created_at'=>now(),'updated_at'=>now()]);
